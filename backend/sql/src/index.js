...
function googleOAuthAudiences() {
  const ids = [];
  // IMPORTANT: Render should set GOOGLE_OAUTH_WEB_CLIENT_ID.
  // Fallback keeps local dev working if env vars are missing.
  const DEFAULT_WEB_CLIENT_ID = "77589502545-jr7a3gfsmrh41oq9igcap9du2uf4n0af.apps.googleusercontent.com";
  const wEnv = normStr(process.env.GOOGLE_OAUTH_WEB_CLIENT_ID);
  const w = wEnv ?? DEFAULT_WEB_CLIENT_ID;
  if (process.env.NODE_ENV === "production" && !wEnv) {
    console.warn("[googleOAuthAudiences] GOOGLE_OAUTH_WEB_CLIENT_ID unset; using default (set env in Render for clarity).");
  }
  if (w) ids.push(w);
...
/** Comma-separated exact origins, e.g. `https://app.vercel.app,http://localhost:5000`. If unset, reflects any Origin (dev-friendly). */
function corsAllowedOrigins() {
  const raw = normStr(process.env.CORS_ALLOWED_ORIGINS);
  if (!raw) return null;
...
app.use(
  cors({
    origin:
      _corsAllow && _corsAllow.length
        ? (origin, callback) => {
            if (!origin) return callback(null, true);
            if (_corsAllow.includes(origin)) return callback(null, true);
